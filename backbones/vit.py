...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj(x).flatten(2).transpose(1, 2)   # num_patches * embed_dim
        return x
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        B = x.shape[0]   # 112
        x = self.patch_embed(x)   # num_patches * embed_dim, 9*768

        cls_tokens = self.cls_token.expand(B, -1, -1)  # embed_dim
